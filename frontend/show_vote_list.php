...
if(isset($_GET['id'])){
    if($_GET['id']==1){
        ...
        if(($now-1)>0){
            $p=$now-1;
            echo "<a href='?do=show_vote_list&id=1&p=$p'><i class='fas fa-angle-double-left'></i></a>";
         }
         ...
         for($i=1;$i<=$pages;$i++){
            ...
            echo "<a href='?do=show_vote_list&id=1&p=$i'>&nbsp;$i&nbsp;</a>";
         }
         
         if(($now+1)<=$pages){
            $p=$now+1;
            echo "<a href='?do=show_vote_list&id=1&p=$p'><i class='fas fa-angle-double-right'></i></a>";
         }
    }
    ...
    if(($now-1)>0){
        $p=$now-1;
        echo "<a href='?do=show_vote_list&p=$p'><i class='fas fa-angle-double-left'></i></a>";
     }
     for($i=1;$i<=$pages;$i++){
        if($i==$now){ //判斷頁碼是否當前頁面
            $fontsize="24px";
        ...
        echo "<a href='?do=show_vote_list&p=$i'>&nbsp;$i&nbsp;</a>";
     }
     
     if(($now+1)<=$pages){
        $p=$now+1;
        echo "<a href='?do=show_vote_list&p=$p'><i class='fas fa-angle-double-right'></i></a>";
     }
}
//控制問卷排序方式 end
